...
- 下列规则分为三类：`硬校验`（系统必须算）、`告警/留痕`（系统不一定拦截，但必须提示并记录）、`治理提醒`（偏组织义务，系统宜做提醒而非阻断）。这三类属于系统产品化分类，不是 FOM Chapter 7 原文直接定义的正式分类；其归纳依据来自 [FOM Chapter 7.pdf](</D:/排班/FOM Chapter 7.pdf>) `7.1.5.2 / Page 9` 中 `must / will` 与 `should / may` 的语义层级。
- 系统展示颜色建议固定为：`硬校验 = 红色`、`告警/留痕 = 橙色`、`治理提醒 = 蓝色`；严重级别 `BLOCK / NON_COMPLIANT / ALERT / INFO` 作为独立维度保留。

## Rule Metadata For System Traceability

本文件是规则整理底稿；当规则落入系统中的 `Rule Catalog / Rule Center / Validation & Publish` 时，每条规则至少应补齐以下结构化字段：

| 字段 | 用途 |
|---|---|
| `rule_id` | 规则唯一标识，用于页面展示、校验结果和例外流程 |
| `rule_category` | `硬校验 / 告警留痕 / 治理提醒`，用于颜色体系和处理方式 |
| `severity_default` | 默认严重级别，供系统初始化展示和动作建议 |
| `source_section` | FOM 所属章节 |
| `source_clause` | FOM 具体条款 |
| `source_page` | PDF 页码 |
| `pdf_deeplink` | 桌面端跳转 PDF 对应页的链接 |

结构化落地原则：

- 同一排班对象可命中多条规则，系统不得只保留最严重一条。
- 每条违规都应显示 `Rule ID + rule_category + source_section / source_clause / source_page`。
- 规则分类用于颜色展示，严重级别用于发布门槛和动作控制，两者不混用。
